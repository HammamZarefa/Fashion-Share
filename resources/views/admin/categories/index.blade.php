                ...
                <div class="card-body col-4">
                    ...
                    <select class="custom-select" name="branch"
                        ...
                        @foreach($sections as $section)
                            <option
                                {{@$id == $section->id ? 'selected' : ''}} value=" {{ route('admin.categories.search',$section->id) }}">
                        ...
                        <table class="table table--light tabstyle--two custom-data-table">
                            ...
                            @forelse ($categories as $item)
                                <tr>
                                    ...
                                    <td data-label="@lang('Image')"><img width="75px"
                                    ...
                                    <td data-label="@lang('Action')">
                                        ...
                                        @else
                                            <a href="javascript:void(0)"
                                               class="icon-btn {{Auth::guard('admin')->user()->branch->categories()->where('branchable_id', $item->id)->exists() ? "btn--danger" :"btn--success"}} ml-1 addBtn"
                                               data-original-title="@lang('Add Or Remove Category From Branch')" data-toggle="tooltip"
                                               data-url="{{ route('admin.categories.add', $item->id ) }}">
                                                <i class="la {{Auth::guard('admin')->user()->branch->categories()->where('branchable_id', $item->id)->exists() ? "la-trash" :"la-check"}}"></i>
                                            </a>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel"> @lang('Add New Category')</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-row form-group">
                            <label class="font-weight-bold ">@lang('Section') <span
                                    class="text-danger">*</span></label>
                            <div class="col-sm-12">
                                <select name="section_id" value="" class="form-control selectpicker"
                            ...
                            <div class="col-sm-12">
                                <select name="section_id" value="" class="form-control selectpicker"
                                    ...
                                    @if(!$categories->isEmpty())
                                        @foreach($sections as $section)
                                            <option
                                                value="{{$section->id}}" {{ $item->section_id == $section->id ? "selected" :""}}>{{ $section->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
